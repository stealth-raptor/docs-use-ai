...
const QuestionListContainer = ({ questionList,onFinish }) => {
    ...
    return (
        <div>
            <div ref={contentRef} className='p-5 border rounded-xl bg-accent mt-5 print-letterhead-space'>
                ...
                <p><b>Patient Weight:</b> {questionList.vitals}</p>
                <br />
                <hr />
                <br />
                ...
                <div>
                    {questionList.prescription?.medications?.map((med, index) => (
                        <p key={index}>
                            {med.generic_name} or {med.brand_names.join(', ')}: {med.dosage_and_frequency} for {med.duration}
                        </p>
                    ))}
                </div>
                <br />
